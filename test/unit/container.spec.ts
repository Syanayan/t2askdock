...
describe('AppContainer', () => {
  it('injects infrastructure implementation into use case', async () => {
    ...
    const container = new AppContainer({
      ...
      databaseProfileRepository: { findById: vi.fn(), findAll: vi.fn().mockResolvedValue([]), save: vi.fn(), delete: vi.fn(), setMode: vi.fn() },
      authStateReader: { isAuthenticated: vi.fn() },
      connectionHealthChecker: { check: vi.fn() },
      osFileAccessChecker: { check: vi.fn(), checkDirectory: vi.fn(), listSqliteFiles: vi.fn() },
      secretStorageService: { saveMountKey: vi.fn(), deleteMountKey: vi.fn(), getMountKey: vi.fn(), saveDirectoryRegistration: vi.fn(), getDirectoryRegistrations: vi.fn().mockResolvedValue([]), deleteDirectoryRegistration: vi.fn() },
      uiEventBus: { subscribe: vi.fn(), publish: vi.fn() } as never,
