...
describe('Phase5 UI integration', () => {
  ...
  it('supports my recent tasks with sort updates', async () => {
    ...
    const listSubtasksByParent = vi.fn().mockResolvedValue([]);
    const countMyTasks = vi.fn().mockResolvedValue(1);
    const provider = new MyRecentTasksProvider({ listMyTasks, listSubtasksByParent, countMyTasks }, 'u1');
    const refresh = vi.fn();
  ...
  it('renders enhanced board html and handles card menu command delegation', async () => {
    ...
    expect(executeCommand).toHaveBeenCalledWith('taskDock.openTaskCreate', expect.objectContaining({ status: 'todo' }));
    expect(executeCommand).toHaveBeenCalledWith('taskDock.openTaskCreate', expect.objectContaining({ status: 'done', title: 'quick create' }));
  });
