...
- [ ] **A-2 `TaskRepository` 等が `ActiveClientHolder` 経由でクライアントを取得するよう変更**
...
- [ ] **A-3 `extension.ts` で `ActiveClientHolder` を初期化**
...
- [ ] **A-4 プロファイル切り替え時に `ActiveClientHolder.switch()` を呼ぶ**
...
- [ ] **A-5 `ActiveClientHolder` のユニットテスト**
...
- [ ] **B-1 `taskDock.selectDatabase` を Quick Pick に変更**
...
- [ ] **C-1 `taskDock.mountDatabase` コマンドを登録**
...
- [ ] **C-2 `package.json` に `taskDock.mountDatabase` を追加**
...
- [ ] **D-1 `taskDock.registerDatabaseDirectory` コマンドを登録**
...
- [ ] **D-2 `package.json` に `taskDock.registerDatabaseDirectory` を追加**
