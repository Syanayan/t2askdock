...
export class TaskRepository implements TaskRepositoryPort {
  ...
  public async listTasksWithDetail(projectId: string): Promise<TaskTreeNode[]> {
    const rows = await this.holder.get().all<{
      taskId: string; title: string; status: Task['value']['status']; priority: Task['value']['priority']; assignee: string | null; progress: number; version: number; parentTaskId: string | null; isClosed:number; isArchived:number;
    }>(`SELECT task_id AS taskId, title, status, priority, assignee, progress, version, parent_task_id AS parentTaskId, is_closed AS isClosed, is_archived AS isArchived FROM tasks WHERE project_id = ? ORDER BY updated_at DESC`, [projectId]);
    ...
    const build = (parentId: string | null): TaskTreeNode[] => (byParent.get(parentId) ?? []).map((r) => ({
      taskId: r.taskId, title: r.title, status: r.status, priority: r.priority, assignee: r.assignee, progress: r.progress, version: r.version, isClosed: r.isClosed===1, isArchived: r.isArchived===1, children: build(r.taskId)
    }));
    return build(null);
  }
