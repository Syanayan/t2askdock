...
export class TaskDetailWebviewPanel {
  ...
  private buildHtml(detail: TaskDetail, subtasks: SubtaskItem[], comments: ReadonlyArray<CommentRow>): string {
    ...
    <div class="layout detail-layout"><div class="main detail-main">
      ...
      <div class="header-actions"><button id="btn-edit" class="btn secondary view-only">Edit</button><button id="btn-save" class="btn edit-only">Save</button><button id="btn-cancel" class="btn secondary edit-only">Cancel</button><button id="btn-close-task" class="btn secondary view-only">Close</button><button id="btn-archive" class="btn secondary view-only">Archive</button><button id="btn-close" class="btn secondary">Close</button></div></div></section>
    ...
    <script>const vscode=acquireVsCodeApi();const orig=${JSON.stringify(detail)};
      ...
      document.getElementById('btn-close-task').onclick=()=>{const reason=prompt('Close reason (required)');if(reason===null)return;const trimmed=reason.trim();if(!trimmed){alert('Close reason is required.');return;}if(confirm('Close this task?'))vscode.postMessage({type:'detail:closeTask',reason:trimmed});};
      document.getElementById('btn-archive').onclick=()=>{if(confirm('Archive this task?'))vscode.postMessage({type:'detail:archiveTask'});};
