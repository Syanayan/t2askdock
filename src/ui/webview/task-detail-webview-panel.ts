...
export class TaskDetailWebviewPanel {
  ...
  public async render(panel: Pick<vscode.WebviewPanel, 'webview' | 'title' | 'dispose'>, taskId?: string, createProjectId?: string): Promise<void> {
    ...
    this.messageListenerDisposable = panel.webview.onDidReceiveMessage(async (message: unknown) => {
      ...
      if (m.type === 'detail:closeWithComment') {
        ...
        await this.addCommentUseCase.execute({ commentId: nextUlid(), taskId, body: reason, actorId: ACTOR_ID, now: new Date().toISOString() });
        panel.dispose();
      }
  ...
  private buildHtml(detail: TaskDetail, subtasks: SubtaskItem[], comments: ReadonlyArray<CommentRow>): string {
    ...
    const updatedAgo = relativeTime(detail.updatedAt);
    const commentRows: CommentRow[] = [...comments].sort((a: CommentRow, b: CommentRow) => a.createdAt.localeCompare(b.createdAt));
    return `<!doctype html><html lang="ja"><head><meta charset="UTF-8"/><style>
    body{background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);font-family:var(--vscode-font-family);margin:0;padding:12px}
    .layout,.detail-layout{display:block}.main,.detail-main{min-width:0}
    .panel{border:1px solid var(--vscode-panel-border);background:var(--vscode-sideBar-background);border-radius:8px;padding:12px;margin-bottom:12px}
    ...
    .field{display:grid;grid-template-columns:120px 1fr;gap:8px;align-items:center;margin-bottom:8px}
    ...
    .comment{display:grid;grid-template-columns:36px 1fr;gap:8px;padding:8px 0;border-bottom:1px solid var(--vscode-panel-border)}
    ...
    .section-label{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;opacity:.8;margin-bottom:10px}
    .props-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}
    .divider{border-top:1px solid var(--vscode-panel-border);margin:10px 0}
    .tag-chip{display:inline-block;border:1px solid var(--vscode-panel-border);border-radius:999px;padding:2px 8px;margin-right:4px;margin-bottom:4px;font-size:11px}
    .comment-avatar{width:30px;height:30px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff}
    .system-comment .comment-body{font-style:italic;color:var(--vscode-descriptionForeground)}
    .comment-actions{display:flex;justify-content:space-between}
    </style></head><body>
    <div class="layout detail-layout"><div class="main detail-main">
      <section class="panel detail-header"><div id="error-banner" style="display:none;color:var(--vscode-errorForeground);margin-bottom:8px"></div><div class="row"><h2 style="margin:0" class="view-only">${safe(detail.title)}</h2><input class="edit-only" id="edit-title" value="${safe(detail.title)}" style="flex:1"/><span class="badge status-${displayStatus}">${statusLabel(displayStatus)}</span><span class="badge priority-${detail.priority}">${priorityLabel(detail.priority)}</span><span style="margin-left:auto"></span><button id="btn-edit" class="btn secondary view-only">Edit</button><button id="btn-save" class="btn edit-only">Save</button><button id="btn-close" class="btn secondary">×</button></div></section>
      <section class="panel"><div class="section-label">Description</div><div class="view-only desc-view" id="desc-view">${safe(detail.description ?? '—')}</div><textarea class="edit-only" id="edit-description" rows="6">${safe(detail.description ?? '')}</textarea></section>
      <section class="panel"><div class="section-label">Task Properties</div>
      ...
      <div class="field"><label>📅 Due Date</label><div class="view-only">${safe(detail.dueDate ?? '—')}</div><input class="edit-only" type="date" id="edit-dueDate" value="${safe(detail.dueDate ?? '')}"/></div>
      <div class="field"><label>🕒 Last Update</label><div>${updatedAgo}</div></div>
      <div class="field"><label>🏷 Label</label><div class="view-only">${detail.tags.length ? detail.tags.map(tag=>`<span class="tag-chip">${safe(tag)}</span>`).join('') : '—'}</div><input class="edit-only" id="edit-tags" value="${safe(detail.tags.join(', '))}"/></div>
      </div>
      <div class="divider"></div>
      <div class="row"><div class="section-label" style="margin:0">Completion Progress</div><span style="margin-left:auto">${detail.progress}%</span></div>
      <div class="view-only"><progress value="${detail.progress}" max="100" style="width:100%"></progress></div><input class="edit-only" id="edit-progress" type="range" min="0" max="100" value="${detail.progress}"/>
      <div class="field edit-only"><label>Status</label><select id="edit-status"><option value="todo">Todo</option><option value="in_progress">In Progress</option><option value="blocked">Blocked</option><option value="done">Done</option></select></div>
      <div class="field edit-only"><label>Priority</label><select id="edit-priority"><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></div>
      </section>
      ...
      <section class="panel"><div class="section-label">💬 Activity</div><div id="comments">${commentRows.map((c: CommentRow)=>renderComment(c, safe)).join('')}</div>
      <textarea id="comment-input" rows="3" placeholder="Type a message..." style="width:100%"></textarea><div class="comment-actions"><button id="btn-comment-close" class="btn secondary">close comment</button><button id="btn-comment-add" class="btn">comment</button></div></section>
    </div></div>
    <script>const vscode=acquireVsCodeApi();const orig=${JSON.stringify(detail)};
      ...
      const rel=(d)=>{const diff=Math.max(0,Date.now()-new Date(d).getTime());const mins=Math.floor(diff/60000);if(mins<60)return mins+' mins ago';const hs=Math.floor(mins/60);if(hs<24)return hs+' h ago';return Math.floor(hs/24)+' days ago';};
      const color=(name)=>'hsl('+(Math.abs([...name].reduce((a,c)=>a+c.charCodeAt(0),0))%360)+' 60% 45%)';
      const init=(name)=>name==='system'?'S':name.trim().slice(0,2).toUpperCase();
      const renderComments=(cs)=>[...cs].filter(c=>!c.deletedAt).sort((a,b)=>a.createdAt.localeCompare(b.createdAt)).map(c=>'<div class=\"comment '+(String(c.createdBy).toLowerCase()==='system'?'system-comment':'')+'\"><div class=\"comment-avatar\" style=\"background:'+color(String(c.createdBy||'U'))+'\">'+esc(init(String(c.createdBy||'U')))+'</div><div><div class=\"meta\"><span class=\"comment-author\">'+esc(c.createdBy)+'</span><span class=\"comment-date\">'+rel(c.createdAt)+'</span></div><div class=\"comment-body\">'+esc(c.body)+'</div></div></div>').join('');
...
function avatarColor(name: string): string {
  ...
  return `hsl(${Math.abs(seed) % 360} 60% 45%)`;
}

function initials(name: string): string {
  if (name.toLowerCase() === 'system') return 'S';
  return name.trim().slice(0, 2).toUpperCase() || 'U';
}

function renderComment(c: CommentRow, safe: (v: string) => string): string {
  return `<div class="comment ${c.createdBy.toLowerCase() === 'system' ? 'system-comment' : ''}">
  <div class="comment-avatar" style="background:${avatarColor(c.createdBy)}">${safe(initials(c.createdBy))}</div>
  <div><div class="meta"><span class="comment-author">${safe(c.createdBy)}</span><span class="comment-date">${relativeTime(c.createdAt)}</span></div><div class="comment-body">${safe(c.body)}</div></div></div>`;
}
