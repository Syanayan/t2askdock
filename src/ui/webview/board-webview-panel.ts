...
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:16px;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground)}.hint{color:var(--vscode-descriptionForeground);font-size:12px}.toolbar{display:flex;justify-content:space-between;gap:8px;margin:8px 0}.toolbar-left,.toolbar-right{display:flex;gap:8px}.toolbar button{border:1px solid var(--vscode-panel-border);background:var(--vscode-button-background);color:var(--vscode-button-foreground);border-radius:6px;padding:5px 10px;font-size:12px;cursor:pointer}.toolbar button[disabled]{opacity:.5;cursor:not-allowed}
#search-box{width:180px;border:1px solid var(--vscode-input-border);background:var(--vscode-input-background);color:var(--vscode-input-foreground);border-radius:6px;padding:5px 10px;font-size:12px}.view-tab{border:1px solid var(--vscode-panel-border);padding:4px 10px;background:var(--vscode-input-background);color:var(--vscode-editor-foreground);cursor:pointer}.view-tab.active{background:var(--vscode-button-background);color:var(--vscode-button-foreground);border-color:var(--vscode-button-background)}
...
.inline-create{display:none;border-radius:8px;border:1px solid var(--vscode-input-border);padding:8px;background:var(--vscode-sideBar-background);margin-bottom:8px}.inline-create.show{display:block}.ic-title{width:100%;box-sizing:border-box}.ic-row{display:flex;gap:4px;margin-top:4px}.ic-row input,.ic-row select,.ic-title{flex:1;font-size:11px;background:var(--vscode-input-background);color:var(--vscode-input-foreground);border:1px solid var(--vscode-input-border);border-radius:4px;padding:4px}.ic-actions{display:flex;gap:4px;margin-top:6px}.ic-actions button{border:1px solid var(--vscode-panel-border);background:var(--vscode-button-background);color:var(--vscode-button-foreground);border-radius:6px;padding:4px 8px}.card-menu{position:relative}.card-menu-btn{border:1px solid var(--vscode-panel-border);background:var(--vscode-toolbar-hoverBackground);color:var(--vscode-editor-foreground);border-radius:4px;padding:0 6px;cursor:pointer}.card-menu-popup{position:absolute;right:0;top:24px;z-index:20;min-width:140px;border:1px solid var(--vscode-menu-border);background:var(--vscode-menu-background);color:var(--vscode-menu-foreground);box-shadow:0 6px 24px rgba(0,0,0,.25);border-radius:6px;padding:4px 0}.card-menu-item{display:block;width:100%;text-align:left;border:0;background:transparent;color:inherit;padding:6px 10px;cursor:pointer}.card-menu-item:hover,.card-menu-item:focus{background:var(--vscode-list-hoverBackground)}body.collapsed .task-desc,body.collapsed .task-meta{display:none}
...
const postCreate=(inline,status)=>{const title=inline.querySelector('.ic-title').value.trim();if(!title){resetInline(inline);return;}const due=inline.querySelector('.ic-due').value.trim();const assignee=inline.querySelector('.ic-assignee').value.trim();const tagsRaw=inline.querySelector('.ic-tags').value.trim();vscode.postMessage({type:'card:create',status,projectId,title,priority:inline.querySelector('.ic-priority').value,dueDate:due||null,assignee:assignee||null,tags:tagsRaw?tagsRaw.split(',').map(t=>t.trim()).filter(Boolean):[]});resetInline(inline)};
const render=()=>{for(const status of statuses){const inStatus=tasks.filter(t=>t.status===status&&matchesFilters(t));const list=document.querySelector('.column[data-status="'+status+'"] .tasks');document.querySelector('.column[data-status="'+status+'"] .count-badge').textContent=String(inStatus.length);list.innerHTML='';for(const task of inStatus){const el=document.createElement('div');el.className='task';const dueClass=task.dueDate&&new Date(task.dueDate).toISOString().slice(0,10)<new Date().toISOString().slice(0,10)?'due-overdue':'';const priorityBadge=task.priority==='medium'?'':'<span class="badge priority-'+task.priority+'">優先度:'+({low:'低',high:'高',critical:'最高'}[task.priority]??'')+'</span>';const statusBadge='<span class="badge status-'+task.status+'">'+statusLabel(task.status)+'</span>';const due=task.dueDate?'<span class="badge '+dueClass+'">'+new Date(task.dueDate).toLocaleDateString('ja-JP')+'</span>':'';el.innerHTML='<div class="task-header"><span class="task-seq">#'+(task.sequenceNumber??'')+'</span><div class="card-menu"><button type="button" class="card-menu-btn" data-action="menu" aria-haspopup="menu" aria-expanded="false">...</button></div></div><div>'+task.title+'</div><div class="task-meta">'+statusBadge+priorityBadge+due+'</div>';el.draggable=true;el.dataset.taskId=task.taskId;el.querySelector('button[data-action="menu"]').addEventListener('click',(e)=>{e.stopPropagation();document.querySelectorAll('.card-menu-popup').forEach(menu=>menu.remove());const wrap=el.querySelector('.card-menu');const button=el.querySelector('button[data-action="menu"]');const popup=document.createElement('div');popup.className='card-menu-popup';popup.setAttribute('role','menu');popup.innerHTML='<button type="button" class="card-menu-item" data-menu-action="edit" role="menuitem">編集</button><button type="button" class="card-menu-item" data-menu-action="delete" role="menuitem">削除</button>';wrap.appendChild(popup);button.setAttribute('aria-expanded','true');const close=()=>{popup.remove();button.setAttribute('aria-expanded','false');};popup.querySelectorAll('[data-menu-action]').forEach(item=>item.addEventListener('click',evt=>{evt.stopPropagation();vscode.postMessage({type:'card:menuAction',action:item.dataset.menuAction,taskId:task.taskId});close();}));popup.addEventListener('keydown',evt=>{if(evt.key==='Escape'){close();button.focus();}});setTimeout(()=>{const onDocClick=(evt)=>{if(!wrap.contains(evt.target)){close();document.removeEventListener('click',onDocClick,true);}};document.addEventListener('click',onDocClick,true);},0);});el.addEventListener('click',()=>vscode.postMessage({type:'card:open',taskId:task.taskId}));el.addEventListener('dragstart',()=>el.dataset.dragging='true');el.addEventListener('dragend',()=>delete el.dataset.dragging);list.appendChild(el);}};renderList();};
...
document.querySelectorAll('.add-task').forEach(button=>button.addEventListener('click',()=>{const inline=button.parentElement.querySelector('.inline-create');inline.classList.add('show');inline.querySelector('.ic-title').focus();}));document.querySelectorAll('.inline-create').forEach(inline=>{const status=inline.closest('.column').dataset.status;const titleInput=inline.querySelector('.ic-title');inline.querySelector('.ic-submit').addEventListener('click',()=>postCreate(inline,status));inline.querySelector('.ic-cancel').addEventListener('click',()=>resetInline(inline));titleInput.addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key==='Enter'){event.preventDefault();postCreate(inline,status);}if(event.key==='Escape'){event.preventDefault();resetInline(inline);}});});
