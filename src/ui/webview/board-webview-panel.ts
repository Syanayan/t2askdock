...
</style></head><body><h2>${projectName ?? 'Task Board'}</h2><div class="toolbar"><div class="toolbar-left"><input type="search" id="search-box" placeholder="検索..."/><button type="button" id="my-tasks-toggle">マイタスク</button></div><div class="toolbar-right view-switcher"><button id="add-task" type="button">Add Task</button><button id="view-kanban" class="view-tab" type="button">カンバン</button><button id="view-list" class="view-tab active" type="button">リスト</button></div></div><section class="board">${['todo','in_progress','blocked','done'].map((s,i)=>`<article class="column" data-status="${s}"><div class="column-header"><h3>${['Todo','In Progress','Blocked','Done'][i]} <span class="count-badge">0</span></h3></div><div class="tasks"></div></article>`).join('')}</section>
...
<script>const vscode=acquireVsCodeApi();let tasks=[];let myTasksOnly=false;let searchQuery='';const saved=vscode.getState();let currentView=saved?.currentView??'list';let projectId=${JSON.stringify(projectId)};const expanded=new Set();let listSort={col:'title',dir:'asc'};const statuses=['todo','in_progress','blocked','done'];const priorityOrder={critical:0,high:1,medium:2,low:3};
const currentUserId=${JSON.stringify(userId ?? 'system')};const matchesFilters=t=>{const q=searchQuery.toLowerCase();const m=!q||t.title.toLowerCase().includes(q)||((t.assignee??'').toLowerCase().includes(q))||((t.tags??[]).some(tag=>tag.toLowerCase().includes(q)));return (!myTasksOnly||t.assignee===currentUserId)&&m;};
...
document.getElementById('search-box').addEventListener('input',e=>{searchQuery=e.target.value;render();});document.getElementById('my-tasks-toggle').addEventListener('click',()=>{myTasksOnly=!myTasksOnly;render();});document.getElementById('view-kanban').addEventListener('click',()=>{currentView='kanban';renderView();});document.getElementById('view-list').addEventListener('click',()=>{currentView='list';renderView();});document.querySelectorAll('th[data-col]').forEach(th=>th.addEventListener('click',()=>{const col=th.dataset.col;if(listSort.col===col){listSort.dir=listSort.dir==='asc'?'desc':'asc';}else{listSort={col,dir:'asc'};}renderList();}));
