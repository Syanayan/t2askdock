...
export class BoardWebviewPanel {
  ...
  public render(panel: Pick<vscode.WebviewPanel, 'webview' | 'title'>, tasks: BoardTask[]): void {
    ...
    panel.webview.html = this.buildHtml(projectId, tasks);
  ...
  private buildHtml(projectId: string | null, initialTasks: BoardTask[]): string {
    const initialPayload = JSON.stringify({
      projectId,
      tasks: initialTasks.map((task, index) => ({ ...task, sequenceNumber: task.sequenceNumber ?? index + 1 }))
    });
    const encodedInitialPayload = Buffer.from(initialPayload, 'utf8').toString('base64');
...
.inline-create{display:none;border-radius:8px;border:1px solid var(--vscode-input-border);padding:8px;background:var(--vscode-editor-background);margin-bottom:8px}.inline-create.show{display:block}.ic-title{width:100%;box-sizing:border-box}.ic-row{display:flex;gap:4px;margin-top:4px}.ic-row input,.ic-row select{flex:1;font-size:11px}.ic-actions{display:flex;gap:4px;margin-top:6px}body.collapsed .task-desc,body.collapsed .task-meta{display:none}
...
<script>const vscode=acquireVsCodeApi();const initialData=JSON.parse(decodeURIComponent(escape(atob('${encodedInitialPayload}'))));let tasks=initialData.tasks??[];let myTasksOnly=false;let searchQuery='';const saved=vscode.getState();let currentView=saved?.currentView??'list';let projectId=initialData.projectId??${JSON.stringify(projectId)};let showDoneInList=false;const expanded=new Set();let listSort={col:'title',dir:'asc'};const statuses=['todo','in_progress','blocked','done'];const priorityOrder={critical:0,high:1,medium:2,low:3};
